import { ArrowDownToLine, Calendar, IndianRupee, LineChart, Trash2, TrendingDown, TrendingUp } from "lucide-react";
...
type Timeframe = "7 Days" | "4 Weeks" | "YTD (Months)" | "12 Months";
...
export function FinancePage() {
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    showToast("success", "Profit Added", `Recorded ${formatCurrency(Number(amount))} for ${date}`);
    setAmount("");
    setNotes("");
  };
  ...
  const stats = useMemo(() => {
    ...
    let today = 0, thisWeek = 0, lastWeek = 0, thisMonth = 0, lastMonth = 0;
    ...
    return { 
      today, 
      thisWeek, growthWeek: getGrowth(thisWeek, lastWeek),
      thisMonth, growthMonth: getGrowth(thisMonth, lastMonth)
    };
  }, [profits]);
    ...
    if (timeframe === "4 Weeks") {
      ...
      profits.forEach((p) => {
        ...
        if (pDate >= new Date(Array.from(map.keys())[0])) {
          const wkStart = new Date(pDate); wkStart.setDate(pDate.getDate() - pDate.getDay());
  ...
  const GrowthBadge = ({ value }: { value: number }) => {
    ...
    const isUp = value > 0;
    return (
      <span className={`ml-2 inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-[10px] font-bold ${
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between animate-fade-in">
        <div>
          <h1 className="font-display text-2xl font-bold text-slate-100">Financial Performance</h1>
          <p className="mt-1 text-sm text-slate-500">Comprehensive overview of your profit margins over time</p>
        </div>
        <button 
      ...
      {showChart && (
        <div className="glass-panel p-5 pt-4 animate-scale-in flex flex-col gap-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <h2 className="text-sm font-semibold text-slate-300">Revenue Trajectory</h2>
            
            {/* Timeframe Selector */}
            <div className="flex rounded-lg bg-dark-700/50 p-1 border border-white/[0.04]">
              {(["7 Days", "4 Weeks", "YTD (Months)", "12 Months"] as Timeframe[]).map((tf) => (
                <button
                  key={tf}
      ...
      <div className="grid gap-6 md:grid-cols-[0.35fr_0.65fr] animate-fade-in stagger-2">
        ...
        <div className="glass-panel p-5 h-fit">
          <h2 className="text-base font-semibold text-slate-200 mb-5">Record New Data</h2>
          <form className="space-y-4" onSubmit={handleAdd}>
            ...
            <div>
              ...
              <input type="number" min="0" step="0.01" className="field font-mono text-emerald-400 font-bold" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} required />
            </div>
            <div>
              <label className="field-label">Source / Notes</label>
              <input type="text" className="field" placeholder="E.g., Event sales, Bulk order" value={notes} onChange={(e) => setNotes(e.target.value)} />
            </div>
            <button type="submit" className="btn-primary w-full mt-2 group">
              <ArrowDownToLine size={16} className="transition-transform group-hover:-translate-y-0.5" /> Commit Record
            </button>
          </form>
        </div>
        ...
        <div className="glass-panel p-0 flex flex-col h-[450px]">
          <div className="p-5 border-b border-white/[0.04] shrink-0 flex justify-between items-center bg-dark-800/50">
            <h2 className="text-base font-semibold text-slate-200">History Ledger</h2>
            <span className="text-xs font-medium text-slate-500">{profits.length} records total</span>
          </div>
          
          <div className="flex-1 overflow-y-auto min-h-0 bg-dark-800/20">
            {profits.length > 0 ? (
              <div className="divide-y divide-white/[0.04]">
                {profits.slice().sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((p) => (
                  <div key={p.id} className="flex items-center justify-between p-4 px-5 hover:bg-dark-750/50 transition-colors group">
                    <div>
